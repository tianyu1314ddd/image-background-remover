            ...
            {subscriptionTiers.map((tier) => (
              ...
              >
                ...
                <div className="mb-4">
                  <span className="text-4xl font-bold text-gray-900">{tier.price}</span>
                  {tier.yearlyNote && (
                    <span className="text-gray-500 text-sm ml-2">{tier.yearlyNote}</span>
                  )}
                </div>
                {tier.monthlyQuota && (
                ...
                <button className="w-full py-3 rounded-lg font-medium bg-purple-600 text-white hover:bg-purple-700 transition-colors">
                  即将推出
                </button>
              </div>
            ))}
        ...
        <div className="mt-12 text-center">
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 max-w-2xl mx-auto">
            <p className="text-yellow-800 text-sm">
              💳 支付方式即将支持 PayPal<br/>
              <span className="text-yellow-600">目前定价仅供参考，购买功能开发中</span>
            </p>
          </div>
        </div>
